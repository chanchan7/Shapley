...
def error(mem):
    ...
    if len(mem) < 100:
        return 1.0
    # 对mem中的数据进行累加,并除以累加的数据点个数,计算得到所有数据点的平均值,得到all_vals
    all_vals = (np.cumsum(mem, 0)/np.reshape(np.arange(1, len(mem)+1), (-1,1)))[-100:]
    # 从all_vals中选取最后100个数据点,用于后续误差的计算,计算这100个数据点与最后数据点的相对误差,计算绝对值的平均值,得到误差errors
